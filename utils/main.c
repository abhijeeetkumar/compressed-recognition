...
int decode_video(
    ...
    pCodecParserCtx=av_parser_init(AV_CODEC_ID_HEVC);  
    if (!pCodecParserCtx){  
...
static PyMethodDef cvaMethods[] = {
    ...
    {NULL, NULL, 0, NULL}        /* Boundary */
};

static struct PyModuleDef cva_module = {
...
PyMODINIT_FUNC PyInit_cva(void)
{
    PyObject *m;

    m = PyModule_Create(&cva_module);
    if (m == NULL)
        return NULL;
    ...
    cvaError = PyErr_NewException("cva.error", NULL, NULL);
    Py_INCREF(cvaError);
    PyModule_AddObject(m, "error", cvaError);
    return m;
}
...
int main(int argc, char *argv[])
{
    ...
    if (program == NULL) {
        fprintf(stderr, "Fatal error: cannot decode the program\n");
        exit(1);
    }

    /* Add a built-in module, before Py_Initialize */
    PyImport_AppendInittab("cva", PyInit_cva);

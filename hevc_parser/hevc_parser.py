...
def parse_video(video_file):
    ffprobe = os.path.normpath(os.path.join(os.path.dirname(os.path.realpath(__file__)), "FFmpeg_x64", "ffprobe"))
    assert_file(ffprobe, "local patched ffprobe could not be found, please run from correct location.")
...
def main(_):
    ...
    parser.add_argument("--report_file", type=str, default="report.json.gz", help="file to store the report, supports compression, e.g. using report.gz")
